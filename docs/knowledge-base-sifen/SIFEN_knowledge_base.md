...
### PROD: WSDL devuelve 302 a /vdesk/hangup.php3 (BIG-IP) — no es tu XML

#### Síntoma
En **PROD** (`sifen.set.gov.py`) al intentar leer el **WSDL** (ej. `...recibe-lote.wsdl?wsdl` o `...consulta-ruc.wsdl`) el HTTP responde:

- **Status:** `302` 
- **Location:** `/vdesk/hangup.php3` 

Esto implica que **no estamos obteniendo WSDL real** (probable protección BIG-IP / portal intermedio).  
En este escenario, **el stack puede fallar antes de enviar** o comportarse inconsistente (porque no puede resolver binding/endpoint desde WSDL).

#### Cómo confirmarlo (diagnóstico)
**Contexto técnico:** La guía oficial recalca **TLS 1.2 con autenticación mutua (mTLS)** para consumir SIFEN. El GET del WSDL puede requerir presentar **certificado cliente** también (no solo el POST SOAP).

1) Asegurar cache folder:
```bash
mkdir -p artifacts/wsdl_cache
```

2) Probar curl simple (sin cliente SOAP):
```bash
curl -k -v https://sifen.set.gov.py/de/ws/async/recibe-lote.wsdl?wsdl
```

3) Probar con mTLS (si el anterior falla):
```bash
curl -vk --cert $SIFEN_CERT_PATH --key $SIFEN_KEY_PATH "https://sifen.set.gov.py/de/ws/async/recibe-lote.wsdl?wsdl"
```

4) Si ves 302 → `/vdesk/hangup.php3`, es BIG-IP bloqueando.  
   El mismo WSDL en TEST funciona:
```bash
curl -k -v https://sifen-test.set.gov.py/de/ws/async/recibe-lote.wsdl?wsdl
```

#### Workaround/ solución
- Si con mTLS funciona: asegurar que el código que hace `GET wsdl_url` incluya `cert=(cert_path,key_path)`.
- Si sigue el 302: habilitar fallback/caché de WSDL (evitar depender de `?wsdl` en PROD).
- Usar WSDL cacheado de TEST o repositorio local para generar stubs/bindings.
- Forzar endpoint correcto en runtime (no confiar en WSDL de PROD).
- Monitorear: si de repente empieza a devolver 200, es que levantaron el bloqueo.

**Nota:** Este síntoma NO indica que tu XML esté mal. Es un problema de conectividad/infraestructura de SIFEN PROD.
