#!/usr/bin/env python3
"""Generar XML con formato indentado para SIFEN"""
...
print("Generando lote con formato indentado...")
...
try:
    ...
    if isinstance(result, tuple):
        ...
        # Parsear y serializar con pretty_print
        root = etree.fromstring(xml_sin_decl)
        xml_pretty = etree.tostring(root, encoding='utf-8', xml_declaration=True, pretty_print=True).decode('utf-8')
        
        print("\n=== XML CON FORMATO ===")
        print(xml_pretty[:1000])
        print("...")
        
        # Crear nuevo ZIP con XML formateado
        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w", compression=zipfile.ZIP_STORED) as z:
            z.writestr("xml_file.xml", xml_pretty.encode('utf-8'))
        zip_bytes_formatted = buf.getvalue()
        
        # Codificar en base64
        zip_base64_formatted = base64.b64encode(zip_bytes_formatted).decode('utf-8')
        
        # Crear payload SOAP
        dId = "01800455473701001000000120260118"  # 15 dígitos
        payload = f'<rEnvioLote xmlns="http://ekuatia.set.gov.py/sifen/xsd"><dId>{dId}</dId><xDE>{zip_base64_formatted}</xDE></rEnvioLote>'
        
        # Enviar con SOAP client
        client = SoapClient(config)
        print("\nEnviando con XML formateado...")
