...
SIFEN_CONS_LOTE_ACTION = "siConsLote"
...
def mtls_download(url: str, out_path: Path, cert_path: str, key_or_password: str, is_pem_mode: bool, cache_dir: Path, debug: bool) -> None:
    ...
    Args:
        ...
        key_or_password: Ruta a la clave (PEM) o contraseña (P12)
    ...
    """
    # Construir comando curl según modo
    cmd = [
        "curl",
...
def load_wsdl_source(wsdl_url: str, cache_dir: Path, wsdl_file: Optional[Path], cert_path: str, key_or_password: str, is_pem_mode: bool, debug: bool) -> Path:
    ...
    try:
        ...
        if debug:
            print(f"⚠️  Error resolviendo imports XSD (continuando sin ellos): {e}")
...
def resolve_wsdl(env: str, wsdl_arg: Optional[str]) -> str:
    ...
    # Prioridad 3: Default según ambiente (con .wsdl)
    if env == "test":
...
def _http_consulta_lote_manual(
    ...
    *,
    ...
    # SOAP 1.2 según XSD WS_SiConsLote_v141.xsd
    # Namespace según Manual Técnico SIFEN v141
...
<soap12:Envelope xmlns:soap12="http://www.w3.org/2003/05/soap-envelope">
    ...
    try:
        req_path.write_bytes(soap_body.encode('utf-8'))
    ...
    # Si el endpoint termina en .wsdl, agregar también la versión sin .wsdl como fallback
    ...
    for endpoint_idx, endpoint in enumerate(endpoints_to_try):
        ...
        for attempt in range(1, 4):  # 3 intentos
            ...
            try:
                ...
                print(f"[SIFEN HTTP] ✅ Éxito con {endpoint_name} (intento {attempt})")
                ...
                error_msg = str(e)
                print(f"[SIFEN HTTP] ❌ Intento {attempt}/3 para {endpoint_name} falló: {type(e).__name__}: {error_msg[:100]}")
                
                # Si es RemoteDisconnected y no es el último intento, continuar con retries
                if "RemoteDisconnected" in error_msg or "ConnectionError" in error_msg:
                    if attempt < 3:
                        continue  # Reintentar
                    else:
                        # Último intento falló, probar siguiente endpoint si hay
                        break
                else:
    ...
    raise RuntimeError(f"Todos los intentos de conexión fallaron. Endpoints probados: {', '.join(endpoints_to_try)}")
...
def call_consulta_lote_raw(
    ...
    try:
        # Endpoint (con .wsdl según config)
        if env == "prod":
...
def _parse_consulta_lote_response(response_xml):
    ...
    try:
        ...
        return {"raw_xml": raw_str, "parse_error": f"XML inválido: {e}", "fallback_shape": "unknown"}
    ...
    # Caso B: rRetEnviDe (errores como 0160 vienen así)
    ...
    # Caso A: rEnviConsLoteDeResponse (shape “ideal” del WSDL)
    if root.xpath('//*[local-name()="rEnviConsLoteDeResponse"]'):
        dFecProc   = _first_text(root, '//*[local-name()="dFecProc"]/text()')
        dCodResLot = _first_text(root, '//*[local-name()="dCodResLot"]/text()')
        dMsgResLot = _first_text(root, '//*[local-name()="dMsgResLot"]/text()')
        # fallback: a veces están como dCodRes/dMsgRes
        if not dCodResLot:
...
def main() -> int:
    ...
    result_dict = None
    fallback_used = False
    history = None  # Declarar aquí para que sea accesible después
    ...
    try:
        ...
        # WSDL/Zeep falló - usar fallback manual
        print(f"\n⚠️  Zeep/WSDL falló ({e}), usando fallback manual...")
        fallback_used = True
        ...
        try:
            ...
            if resp_elem is not None:
                ...
                try:
                    ...
                    if resp_a:
                        # Dejar que el flujo existente siga (no tocar acá)
                        result_dict = {"raw_xml": response_xml}
                    else:
                        # Caso B (común en errores): rRetEnviDe
                        ...
                        if resp_b:
                            ...
                            if resp_c:
                                ...
                                result_dict = {"raw_xml": response_xml, "parse_error": "No se encontró rEnviConsLoteDeResponse, rRetEnviDe ni rResEnviConsLoteDe"}
                except Exception as e:
            ...
            print(f"\n⚠️  Error en fallback manual: {fallback_error}")
            print("ℹ️  Generando JSON de error...")
    ...
    try:
        ...
        if "error_type" in resp_dict:
            ...
            # Respuesta SOAP exitosa (aunque pueda tener código de error SIFEN)
        ...
        # Guardar XML de respuesta si está disponible (desde history plugin o fallback)
        if debug_enabled:
            if fallback_used and "raw_xml" in resp_dict:
                ...
                print(f"💾 Artifact guardado (fallback): {fname.name}")
        ...
        if hasattr(history, 'last_sent') and history.last_sent:
            ...
            if sent_envelope:
                ...
                try:
                    ...
                    print(f"⚠️  No se pudo guardar headers: {e}")
        
    except Exception as e:
        # En caso de error parsing, guardar JSON de error
        print(f"\n⚠️  Error al parsear respuesta: {e}")
        print("ℹ️  Generando JSON de error...")
