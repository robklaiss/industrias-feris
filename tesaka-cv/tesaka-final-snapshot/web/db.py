...
def insert_document(cdc: str, ruc_emisor: str, timbrado: str, de_xml: str) -> int:
    ...
    try:
        ...
        # Si el CDC ya existe, devolver el id existente (evita 500 en /de/new)
        if "UNIQUE constraint failed: de_documents.cdc" in str(e):
            try:
                cursor.execute("SELECT id FROM de_documents WHERE cdc = ?", (cdc,))
                row = cursor.fetchone()
...
def update_document_status(
    ...
    # aliases flexibles para compatibilidad hacia atrás/adelante
    ...
    Args:
        ...
        last_status / status: Nuevo estado (uno de los estados válidos de document_status).
        last_code / code: Código de respuesta de SIFEN (opcional).
        last_message / message: Mensaje de respuesta de SIFEN (opcional).
    ...
    """
    # Resolver aliases: status -> last_status, code -> last_code, message -> last_message
    effective_status = last_status if last_status is not None else status
    effective_code = last_code if last_code is not None else code
    effective_message = last_message if last_message is not None else message

    if effective_status is None:
        raise ValueError("update_document_status requiere 'status' o 'last_status'")
    ...
    try:
        ...
        params = [effective_status, updated_at]

        if effective_code is not None:
            updates.append("last_code = ?")
            params.append(effective_code)

        if effective_message is not None:
            updates.append("last_message = ?")
            params.append(effective_message)
