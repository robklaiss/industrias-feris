#!/usr/bin/env python3
"""Generar un lote XML fresco para probar el fix del doble wrapper"""
...
if isinstance(result, tuple):
    ...
    if '<rLoteDE><rLoteDE' in xml_content:
        print("❌ AÚN TIENE DOBLE WRAPPER!")
    elif '<rLoteDE xmlns=' in xml_content and xml_content.count('<rLoteDE>') == 1:
        print("✅ ESTRUCTURA CORRECTA: Un solo <rLoteDE>")
    else:
        print("⚠️  ESTRUCTURA INESPERADA")
        
    # Guardar para enviar
    with open('payload_fresco.txt', 'w') as f:
        # Extraer solo el lote sin XML declaration
        import re
        lote_sin_decl = re.sub(r'^\s*<\?xml[^>]*\?>\s*', '', xml_content)
        f.write(lote_sin_decl)
    
    print("\nPayload guardado en payload_fresco.txt")
else:
    print("Error al generar lote")
