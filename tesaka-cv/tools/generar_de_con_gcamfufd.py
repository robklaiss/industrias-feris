...
incluyendo gCamFuFD después de Signature.
"""
...
def generar_de_con_gcamfufd():
    # Generar IDs únicos
    de_id = f"018004554737010010000001{datetime.now().strftime('%Y%m%d%H%M%S')}123456789"
    rde_id = f"rDE{de_id}"
    
    # Crear rDE con Id y xsi:schemaLocation
    ...
    # NO agregamos Signature - que el sistema lo firme
    
    # Agregar gCamFuFD como hijo de rDE (fuera de DE)
    # Esto es importante: gCamFuFD debe ser hijo de rDE, no de DE
...
if __name__ == '__main__':
    xml_bytes = generar_de_con_gcamfufd()
    
    # Guardar
    output_file = sys.argv[1] if len(sys.argv) > 1 else 'test_rde_con_gcamfufd.xml'
    ...
    print(f"- gCamFuFD presente: {'YES' if rde.find('.//s:gCamFuFD', ns) is not None else 'NO'}")
    print(f"- gCamFuFD fuera de DE: {'YES' if rde.find('./s:gCamFuFD', ns) is not None else 'NO'}")
