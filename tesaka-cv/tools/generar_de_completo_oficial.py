...
- rDE con Id único y xsi:schemaLocation
- dVerFor como primer hijo
- Estructura completa con gOpeCom, gCamFE, gCamCond
...
def generar_de_completo():
    ...
    etree.SubElement(de, f'{{{SIFEN_NS}}}dFecFirma').text = '2020-05-01T14:01:50'
    ...
    # gDatGralOpe con gOpeCom (FALTANTE ANTES)
    gDatGralOpe = etree.SubElement(de, f'{{{SIFEN_NS}}}gDatGralOpe')
    etree.SubElement(gDatGralOpe, f'{{{SIFEN_NS}}}dFeEmiDE').text = '2020-05-07T15:03:57'
    ...
    # gDtipDE con gCamFE y gCamCond (FALTANTES ANTES)
    ...
    # gCamItem 2
    gCamItem2 = etree.SubElement(gDtipDE, f'{{{SIFEN_NS}}}gCamItem')
    etree.SubElement(gCamItem2, f'{{{SIFEN_NS}}}dCodInt').text = 'TAU/TAUR'
    etree.SubElement(gCamItem2, f'{{{SIFEN_NS}}}dDesProSer').text = 'TARJETA URGENTE'
    etree.SubElement(gCamItem2, f'{{{SIFEN_NS}}}cUniMed').text = '77'
    etree.SubElement(gCamItem2, f'{{{SIFEN_NS}}}dDesUniMed').text = 'UNI'
    etree.SubElement(gCamItem2, f'{{{SIFEN_NS}}}dCantProSer').text = '1'
    
    gValorItem2 = etree.SubElement(gCamItem2, f'{{{SIFEN_NS}}}gValorItem')
    etree.SubElement(gValorItem2, f'{{{SIFEN_NS}}}dPUniProSer').text = '1100000'
    etree.SubElement(gValorItem2, f'{{{SIFEN_NS}}}dTotBruOpeItem').text = '1100000'
    
    gValorRestaItem2 = etree.SubElement(gValorItem2, f'{{{SIFEN_NS}}}gValorRestaItem')
    etree.SubElement(gValorRestaItem2, f'{{{SIFEN_NS}}}dDescItem').text = '0'
    etree.SubElement(gValorRestaItem2, f'{{{SIFEN_NS}}}dPorcDesIt').text = '0'
    etree.SubElement(gValorRestaItem2, f'{{{SIFEN_NS}}}dDescGloItem').text = '0'
    etree.SubElement(gValorRestaItem2, f'{{{SIFEN_NS}}}dTotOpeItem').text = '1100000'
    
    gCamIVA2 = etree.SubElement(gCamItem2, f'{{{SIFEN_NS}}}gCamIVA')
    etree.SubElement(gCamIVA2, f'{{{SIFEN_NS}}}iAfecIVA').text = '1'
    etree.SubElement(gCamIVA2, f'{{{SIFEN_NS}}}dDesAfecIVA').text = 'Gravado IVA'
    etree.SubElement(gCamIVA2, f'{{{SIFEN_NS}}}dPropIVA').text = '100'
    etree.SubElement(gCamIVA2, f'{{{SIFEN_NS}}}dTasaIVA').text = '10'
    etree.SubElement(gCamIVA2, f'{{{SIFEN_NS}}}dBasGravIVA').text = '1000000'
    etree.SubElement(gCamIVA2, f'{{{SIFEN_NS}}}dLiqIVAItem').text = '100000'
    
    # gCamEsp
    gCamEsp = etree.SubElement(gDtipDE, f'{{{SIFEN_NS}}}gCamEsp')
    gGrupAdi = etree.SubElement(gCamEsp, f'{{{SIFEN_NS}}}gGrupAdi')
    etree.SubElement(gGrupAdi, f'{{{SIFEN_NS}}}dCiclo').text = 'DICIEMBRE'
    etree.SubElement(gGrupAdi, f'{{{SIFEN_NS}}}dFecIniC').text = '2018-11-23'
    etree.SubElement(gGrupAdi, f'{{{SIFEN_NS}}}dFecFinC').text = '2018-12-17'
    
    # gTransp
    gTransp = etree.SubElement(gDtipDE, f'{{{SIFEN_NS}}}gTransp')
    etree.SubElement(gTransp, f'{{{SIFEN_NS}}}iModTrans').text = '1'
    etree.SubElement(gTransp, f'{{{SIFEN_NS}}}dDesModTrans').text = 'Terrestre'
    etree.SubElement(gTransp, f'{{{SIFEN_NS}}}iRespFlete').text = '2'
    etree.SubElement(gTransp, f'{{{SIFEN_NS}}}dNuDespImp').text = '1234567891asfdcs'
    ...
    # Signature con xmlns XMLDSig estándar (como en el ejemplo)
    sig = etree.SubElement(rde, 'Signature', xmlns='http://www.w3.org/2000/09/xmldsig#')
    
    # gCamFuFD después de Signature
    gCamFuFD = etree.SubElement(rde, f'{{{SIFEN_NS}}}gCamFuFD')
    etree.SubElement(gCamFuFD, f'{{{SIFEN_NS}}}dCarQR').text = 'https://ekuatia.set.gov.py/consultas-test/qr?nVersion=150&amp;Id=01000000019001001100005022020050710000000231&amp;dFeEmiDE=323032302d30352d30375431353a30333a3537&amp;dRucRec=00000002&amp;dTotGralOpe=2200000&amp;dTotIVA=200000&amp;cItems=2&amp;DigestValue=346b6f4a61715857704f6e64334971754d5331733776626e426149705973316435586a53572b6e337549343d&amp;IdCSC=0001&amp;cHashQR=67b07c782077eef44677e4d2f239cb111aea97d6f6ba177188eed0e06e6d3df9'
...
if __name__ == '__main__':
    xml_bytes = generar_de_completo()
    
    # Guardar
    output_file = sys.argv[1] if len(sys.argv) > 1 else 'test_rde_completo_oficial.xml'
